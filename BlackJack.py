...
def startGamemMain(dealer: list,player: list):
    ...
    drawAnotherCard = input("Hit or Stand ?")
    lost = False
    while (drawAnotherCard.lower() == "hit"):
        player.append(generateCard())

        if (sum(player) > 21):
            showCards(True,dealer,player)
            lost = True
            break
        else:
            showCards(False,dealer,player)
    ...
    if (sum(dealer) < 17) : dealer.append(generateCard())
...
#can add loop for many games and scrore and such....
startGamemMain([],[])
